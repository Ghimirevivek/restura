    ...
    >
      {data.length > 0 ? (
        data.map((item) => (
          <div
            key={item.id}
            className='min-w-210 w-275 h-250 md:w-300 bg-cardOverlay rounded-lg py-2 px-4  my-12 backdrop-blur-lg hover:drop-shadow-lg flex flex-col items-center justify-evenly relative'
          >
            <div className='w-full flex items-center justify-between'>
              <motion.img
                ...
                src={item.imageURL}
                alt='imgurl'
                className='w-[80%] h-150 object-contain'
              />
              <motion.div
                whileTap={{ scale: 0.75 }}
                className='w-8 h-8 rounded-full bg-red-600 flex items-center justify-center'
              >
                <MdShoppingBasket className='text-white text-lg cursor-pointer' />
              </motion.div>
            </div>
